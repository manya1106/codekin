...
import type { UserProgress, RoadmapProgress } from "@codekin/shared";
import { emptyProgress, type ProblemRecord } from "@codekin/shared";
import { db } from "./firebase";

// Rough estimates for topic totals (used to compute mastery %)
const TOPIC_ESTIMATES: Record<string, number> = {
...
export function useProgress(uid?: string) {
  ...
  const [weakTopic, setWeakTopic] = useState<string>("Binary Search");
  ...
  useEffect(() => {
    ...
    const unsubscribeProgress = onSnapshot(progressRef, { includeMetadataChanges: true }, (snapshot) => {
      const data = snapshot.exists() ? snapshot.data() as UserProgress : emptyProgress;
      setProgress(data);
    ...
    const unsubscribeRecent = onSnapshot(recentQuery, { includeMetadataChanges: true }, (snapshot) => {
      const docs = snapshot.docs.map((d) => d.data() as ProblemRecord);
      if (!snapshot.metadata.fromCache) setLive(true);
      ...
      if (topicCounts.size > 0) {
        const roadmapData: RoadmapProgress[] = [...topicCounts.entries()]
          .sort((a, b) => b[1] - a[1])
          .slice(0, 8)
          .map(([topic, solved]) => ({
            topicId: topic,
