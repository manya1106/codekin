...
async function attempted(slug) { return Boolean((await chrome.storage.local.get(`attempted:${slug}`))[`attempted:${slug}`]); }
...
async function applyBlindMode() {
    ...
    if (state.blindMode === false || await attempted(slug()))
        return;
...
function watchSubmissions() {
    ...
    document.addEventListener("click", (event) => {
        ...
        if (label === "Run" || label === "Run Code" || label === "Submit")
            reveal();
        if (label === "Submit")
    ...
    new MutationObserver((mutations) => {
        ...
        if (result !== "Accepted" || seen.has(slug()))
            return;
        seen.add(slug());
        const title = document.querySelector("a[href^='/problems/']")?.textContent?.replace(/^\d+\.\s*/, "").trim() || document.title.split(" - ")[0];
        const problem = { slug: slug(), title, difficulty: inferDifficulty(), topics: [], status: "solved", solvedAt: new Date().toISOString(), source: "extension" };
        void chrome.runtime.sendMessage({ type: "QUEUE_SOLVE", problem });
        const copy = document.querySelector("#ck-copy");
        if (copy)
            copy.innerHTML = `Accepted! <b>Nice work, +XP earned.</b>`;
