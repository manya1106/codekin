...
async function applyBlindMode() {
    if (!await isBlindModeEnabled())
        return;
    // On problem pages, skip if already attempted
    if (isOnProblemPage() && await attempted(currentSlug()))
        return;
...
function watchSubmissions() {
    ...
    new MutationObserver((mutations) => {
        ...
        if (result !== "Accepted" || seen.has(currentSlug()))
            return;
        seen.add(currentSlug());
        const title = document.querySelector("a[href^='/problems/']")?.textContent?.replace(/^\d+\.\s*/, "").trim() ||
