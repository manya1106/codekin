...
async function fetchProblemMeta(slug) {
    try {
        const data = await leetcodeGql(`query questionData($titleSlug: String!) {
...
async function batchFetchMeta(slugs) {
...
async function connect() {
    ...
    if (!clientId || clientId.includes("REPLACE_WITH"))
        throw new Error("Chrome OAuth client ID is not configured");
...
async function flush() {
    const result = { written: 0, skipped: 0, failed: [], status: "empty" };
    if (!db || !auth)
    ...
    try {
        const existingChecks = await Promise.all(queue.map((problem) => getDoc(doc(db, "users", uid, "problems", problem.slug))));
...
async function repairStoredMetadata() {
    const result = { checked: 0, repaired: 0, status: "empty", errors: [] };
    if (!db || !auth)
        return { ...result, status: "not_connected" };
    await auth.authStateReady();
    if (!auth.currentUser)
    ...
    try {
        ...
        const batch = writeBatch(db);
        let totalXp = 0;
        let latestSolvedAt = null;
        for (const item of problems) {
            const meta = metaMap.get(item.id);
            const nextProblem = meta
                ? { ...item.data, difficulty: meta.difficulty, topics: meta.topics, metadataSource: "leetcode" }
                : item.data;
            ...
            const changed = meta && (item.data.difficulty !== meta.difficulty ||
        ...
        if (result.repaired > 0) {
            ...
            const progressRef = doc(db, "users", uid, "private", "progress");
            const current = (await getDoc(progressRef)).data();
            batch.set(progressRef, {
...
async function syncHistory() {
    const result = { fetched: 0, queued: 0, alreadyKnown: 0, metadataFetched: 0, flushed: 0, status: "queued_locally", errors: [] };
    ...
    const userData = await leetcodeGql("{ userStatus { username } }");
    ...
    const subData = await leetcodeGql(`query recentAcSubmissions($username: String!, $limit: Int!) {
...
chrome.runtime.onMessage.addListener((message, _sender, respond) => {
    ...
    if (message.type === "CONNECT") {
        connect()
            ...
            try {
                ...
                syncResult = { fetched: 0, queued: 0, alreadyKnown: 0, metadataFetched: 0, flushed: 0, status: "queued_locally", errors: [e.message] };
            }
    ...
    if (message.type === "SYNC_HISTORY") {
        chrome.storage.local.set({ forceHistorySync: true }).then(() => syncHistory()
