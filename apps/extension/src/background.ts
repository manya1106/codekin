...
import { evolutionForLevel, levelFromXp, XP_BY_DIFFICULTY, type ProblemRecord, type UserProgress } from "@codekin/shared";
...
async function flush() {
  ...
  const uid = auth.currentUser.uid; const remaining = [...queue];
  for (const problem of queue) {
    ...
    await runTransaction(db, async (tx) => {
      ...
      if (existingProblem.data()?.status === "solved") return;
...
chrome.runtime.onMessage.addListener((message, _sender, respond) => {
  if (message.type === "CONNECT") connect().then((u) => respond({ ok: true, name: u?.displayName })).catch((e) => respond({ ok: false, error: e.message }));
  if (message.type === "QUEUE_SOLVE") getQueue().then(async (queue) => { if (!queue.some((x) => x.slug === message.problem.slug)) queue.push(message.problem); await chrome.storage.local.set({ outbox: queue }); await flush(); respond({ ok: true }); });
  if (message.type === "FLUSH") flush().then(() => respond({ ok: true })).catch((e) => respond({ ok: false, error: e.message }));
  return true;
});

chrome.alarms.create("sync-outbox", { periodInMinutes: 5 });
chrome.alarms.onAlarm.addListener((alarm) => { if (alarm.name === "sync-outbox") void flush(); });
